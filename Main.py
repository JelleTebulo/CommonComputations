...
# x_s = 13.6
# z_s = 2
# my_s = - da2 - 2*np.arctan((2*L2*np.tan(b/2) - np.sqrt(2)*np.sqrt(-(-2*L2**2 + 2*L3**2 - 4*L3*x_s*np.cos(b) - 4*L3*z_s*np.sin(b) + x_s**2*np.cos(2*b)
#         + x_s**2 + 2*x_s*z_s*np.sin(2*b) - z_s**2*np.cos(2*b) + z_s**2)/(np.cos(b) + 1)**2))/(L2*np.tan(b/2)**2 - L2 + L3*np.tan(b/2)**2 + L3 + x_s*np.tan(b/2)**2
#         - x_s - 2*z_s*np.tan(b/2)))
# my_s = np.max(my_s)
# print(f'my_s = {np.rad2deg(my_s)}')
